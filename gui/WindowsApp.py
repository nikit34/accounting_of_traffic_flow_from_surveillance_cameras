...
def butd(pf, bu1, bu2, pan1, pan2):
    pathf = '..' + os.sep + 'data' + os.sep + 'cin' + os.sep + pf + ':' + pan1 + ':' + pan2
    global image1
    image1 = ImageTk.PhotoImage(file="СТАРТ1.png")
    button1 = Button(bu1, image=image1, bg='gray', borderwidth=2, relief="raised",
                     command=lambda: writestr(pathf, True))
    button1.place(relx=0, rely=0, relwidth=1, relheight=1)
    global image2
    image2 = ImageTk.PhotoImage(file="ТРАЕКТОРИЯ.png")
    button2 = Button(bu2, image=image2, bg='gray', borderwidth=2, relief="raised",
                     command=lambda: writestr(pathf, False))
...
def main():
    ...
    button5 = Button(root, image=image5, bg='gray', borderwidth=2, relief="raised",
                     command=lambda: butd(str(entry.get()), bu1, bu2, str(entry01.get()), str(entry02.get())))
